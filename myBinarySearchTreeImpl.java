...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	//My implementation has the root node being at level 1.
	public int my_count_at_level(T1 level){
		myBinarySearchNode<T1, T2> res = my_root();
		//local class to keep track of the level, depth and the number of nodes
		class Count{
			public int my_count(myBinarySearchNode<T1, T2> res ,T1 level, int depth, int count){
				//if the node is null then return 0
				if(res == null) return 0;
				//if the overall depth is equal to the level increment count
				else if(depth  == (int)level) count ++;
				else{
					//recursively calls my_count with left node and right node and returns the sum of the nodes
					return my_count(res.getLeft(), level, depth + 1, count) + my_count(res.getRight() , level, depth + 1, count);
				}
				return count;
			}
		}
		int count = new Count().my_count(res, level, 1, 0);
	...
	public boolean my_is_balanced(){
		if(this.root == null){
			...
			int leftHeight = my_left_tree().my_length();
			//gets the height of the right tree
			int rightHeight = my_right_tree().my_length();

			//gets the absolute value of left-right and checks if its greater than 1
			if(Math.abs(leftHeight - rightHeight) > 1) return false;
			//recursively call the is balanced with the right subtree and left subtree

			//Both must be true for it to return true
			return (this.my_right_tree().my_is_balanced() && this.my_left_tree().my_is_balanced());
		}
	}
	...
	public int my_count_smaller_nodes(T1 key){
		...
		// if key is equal to the root then count all of the left subtree
		else if(key.compareTo(this.root.getKey()) == 0){
			count+= this.my_left_tree().my_node_count();
		}
		// if smaller than root then call function with new left subtree
		else if(key.compareTo(this.root.getKey()) < 0){
			return this.my_left_tree().my_count_smaller_nodes(key);

		} 
		// if greater than root then check both subtrees 
		else if(key.compareTo(this.root.getKey()) > 0){
			count+= this.my_left_tree().my_node_count() + this.my_right_tree().my_count_smaller_nodes(key) + 1;
		}
	...
	public int my_find_node_at_level(T1 key){
		myBinarySearchNode<T1, T2> res = my_root();
		//Local class to find the level
		class Level{
			public int my_find_node(myBinarySearchNode<T1, T2> res ,T1 key, int depth){
				if(res == null) return -1;
				else{
					//if the key is less than the node then recursively check the left node
					if(key.compareTo(res.getKey()) < 0) return my_find_node(res.getLeft(), key, depth + 1);

					//if the key is greater than the node then recursively check the right node
					else if (key.compareTo(res.getKey()) > 0) return my_find_node(res.getRight(), key, depth + 1);

					// if its equal then return the depth + 1
					else return depth + 1;
				}
			}	
		}
		// returns the depth
		return new Level().my_find_node(res, key, 0);
	}
}
